...
exports.onSlideInputMeeting = functions.database
  ...
  .onWrite(async (change, context) => {
    ...
    await db
      .ref(
        `/data/slides/${context.params.meetingid}/_history/${timestamp}/${context.params.field}`
      )
...
exports.onSlideInputSequence = functions.database
  ...
  .onWrite(async (change, context) => {
    ...
    await db
      .ref(
        `/sequence/${context.params.sequenceid}/_history/${timestamp}/${context.params.field}`
      )
      .set(change.after.val());
  });
